...
- **Student page** — `assignments/assignment.html?id=<id>` renders each problem (goal GIF, editor, instant check) and, at the page bottom, **one combined submission textarea** covering all three. Every problem's code is auto-framed in an outer function (`function problem_<n>() { … return main; }`, numbered by position) so the single file a student copies contains all three solutions and a grader can run each independently. Assignments are listed at `assignments/index.html`.
- **Teacher grader** — `tools/grade.js` reads a roster, pulls each student repo, and grades every submission in an isolated worker (with a timeout, so an infinite loop can't hang the run), emitting `gradebook.json` and a printed table. It handles both shapes: a standalone single-problem assignment (a bare `main` or a single framed `problem_1()`), and a lesson assignment (it unwraps each framed `problem_<n>()` and awards per-problem points).
