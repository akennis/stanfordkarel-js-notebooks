...
- **LLM review is advisory and orthogonal to the grade.** `tools/llm-review.js` (called from `grade.js` only when `--llm-endpoint` **and** `--llm-model` are given) POSTs the problem prompt, the golden solution plaintext, the submitted code (with the `problem_<n>` framing and its `return main` stripped, so the model sees the program as authored), and the deterministic verdict to an OpenAI-compatible chat endpoint and prints a prose code-quality critique to **stdout** (progress stays on stderr; the score table is unchanged). The system prompt speaks in first person as the course instructor and tells the model to ignore indentation. It runs for every graded problem, per problem for lesson assignments. It never changes points, and any endpoint failure is logged to stderr and skipped — keep it that way. Dependency-free (Node global `fetch`).
